...
if (volume > 0) and (volume2 > 0) and (volume2 > volume) and (time_elapsed.days > 0):
    ...
    st.text_area("Befundtext", value = f"Am {my_date} betrug das Volumen {round(volume)} ml, {time_elapsed.days} Tage später, am {my_date2} betrug das Volumen {round(volume2)}. Dies enspricht einer Volumenverdoppelungszeit von {VDT} Tagen.")
else:
